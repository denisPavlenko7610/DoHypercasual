...
namespace AssetStore.MyBike
{
    public class MyBikeControll : MonoBehaviour
    {
        ...
        {
            0.6f, 0.65f, 0.7f, 0.75f, 0.8f, 0.85f, 0.9f, 1.0f, 1.0f, 0.95f, 0.80f, 0.70f, 0.60f, 0.5f, 0.45f, 0.40f,
            0.36f,
        ...
        private WheelComponent SetWheelComponent(Transform wheel, Transform axle, bool drive, float maxSteer,
            float pos_y)
        {
        ...
        void FixedUpdate()
        {
            ...
            if (activeControl)
            {
                ...
                steer = Mathf.MoveTowards(steer, joystick.Horizontal, 0.1f);
                accel = joystick.Vertical;
            ...
            else if (bikeSetting.automaticGear && (motorRPM > bikeSetting.shiftUpRPM) && (accel > 0.0f) &&
                     speed > 10.0f &&
                     !brake)
            {
                ShiftUp();
            ...
            foreach (WheelComponent w in wheels)
            {
                ...
                {
                    lp.y -= Vector3.Dot(w.wheel.position - hit.point, transform.TransformDirection(0, 1, 0)) -
                            (col.radius);
